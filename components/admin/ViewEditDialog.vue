<template>
  <BaseDialog
    v-model="isOpen"
    title="Edit View"
    subtitle="Update calendar view configuration"
...
const handleSubmit = async () => {
  ...
  loading.value = true;
  try {
    await $fetch(`/api/views/${props.view.id}`, {
      method: "PUT",
      body: form.value,
    });
...
const loadFormData = () => {
  if (props.view) {
    form.value = {
      ...
      alias: props.view.alias,
      selectedCalendarIds: [...props.view.selectedCalendarIds],
      columnCount: props.view.columnCount,
      paddingPx: props.view.paddingPx,
    };
  }
};
